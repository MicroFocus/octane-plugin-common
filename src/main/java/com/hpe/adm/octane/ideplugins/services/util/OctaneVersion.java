...
public class OctaneVersion implements Comparable<OctaneVersion> {
    ...
    public OctaneVersion(String versionString) {
        ...
        if (!(parts.length == 3 || parts.length == 4)) {
            throw new RuntimeException("Unable to parse OpenText™️ Core Software Delivery Platform version from string: " + versionString);
        }
        ...
        try {
            ...
            throw new RuntimeException("Unable to parse OpenText™️ Core Software Delivery Platform version from string: " + versionString, ex);
        }
    }
